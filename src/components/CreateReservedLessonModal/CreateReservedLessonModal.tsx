    ...
    if (window.confirm(confirmMessage)) {
      try {
        ...
        if (data.response.checkout_url) {
          reservedLessonsAPI.create(payload)
          window.open(data.response.checkout_url, "_self")
        } else {
    ...
    >
      {!selectedTime ? (
        ...
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          <p>
            Початок уроку: <b>{customDayjs(selectedTime).format("DD.MM.YYYY - HH:mm")}</b>.
            Тривалість уроку: <b>{selectedLesson.duration} хв.</b>
          </p>
          <Button onClick={onBookLessons}>Підтвердити</Button>
        </div>
      )}
      <Calendar selectable events={calendarEvents} onSelectLessonsTime={setSelectedTime} />
    </Dialog>
  )
}
