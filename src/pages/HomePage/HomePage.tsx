      ...
      <div className={styles.container}>
        <div className={styles.wrapper}>
          <div className={styles.tutors}>
            {loadingStatus === LoadingStatusTypes.LOADING && <LoadingSpinner />}
            {loadingStatus === LoadingStatusTypes.ERROR && <Error />}

            {lessons && !lessons.length && loadingStatus === LoadingStatusTypes.SUCCESS ? (
              <h3 style={{ textAlign: "center" }}>
                За вказаними критеріями репетиторів не знайдено
              </h3>
            ) : (
              lessons &&
